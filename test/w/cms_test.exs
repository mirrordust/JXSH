defmodule W.CMSTest do
  ...
  describe "images" do
    ...
    @valid_attrs %{location: "some location", name: "some name", size: 120.5}
  ...
  describe "files" do
    ...
    test "list_files/0 returns all files" do
      file = file_fixture()
      assert CMS.list_files() == [file]
    end

    test "get_file!/1 returns the file with given id" do
      file = file_fixture()
      assert CMS.get_file!(file.id) == file
    end

    test "create_file/1 with valid data creates a file" do
      assert {:ok, %File{} = file} = CMS.create_file(@valid_attrs)
      assert file.name == "some name"
    end

    test "create_file/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = CMS.create_file(@invalid_attrs)
    end

    test "update_file/2 with valid data updates the file" do
      file = file_fixture()
      assert {:ok, %File{} = file} = CMS.update_file(file, @update_attrs)
      assert file.name == "some updated name"
    end

    test "update_file/2 with invalid data returns error changeset" do
      file = file_fixture()
      assert {:error, %Ecto.Changeset{}} = CMS.update_file(file, @invalid_attrs)
      assert file == CMS.get_file!(file.id)
    end

    test "delete_file/1 deletes the file" do
      file = file_fixture()
      assert {:ok, %File{}} = CMS.delete_file(file)
      assert_raise Ecto.NoResultsError, fn -> CMS.get_file!(file.id) end
    end

    test "change_file/1 returns a file changeset" do
      file = file_fixture()
      assert %Ecto.Changeset{} = CMS.change_file(file)
    end
  end
end
